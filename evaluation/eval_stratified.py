...
def run_evaluation(dataset_dir: str):
    gt_file = os.path.join(dataset_dir, "stratified_manifest.json")
    if not os.path.exists(gt_file):
        print(f"Manifest not found at {gt_file}")
        return
    ...
    run_id = f"stratified_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
    provider = "unknown"
    ...
    with open(os.path.join(dataset_dir, "eval_stratified_results.json"), "w") as f:
