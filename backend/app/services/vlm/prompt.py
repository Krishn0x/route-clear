...
IMPORTANT: Every required field in the JSON schema MUST be present in the output. For boolean fields like signature_present and correction_detected, output the field with a boolean value if determined, or `null` if the field is absent or its status is unreadable. Do not omit the key.
